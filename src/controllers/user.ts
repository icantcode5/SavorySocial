...
export async function registerUser(request: Request, response: Response):Promise<void> {
	...
	try {
		...
		const userArrayWithoutPassword = user.rows.map((obj) => ({user_id: obj.user_id, name: obj.name, email:obj.email, accessToken : accessToken }))
		response.status(200).send(userArrayWithoutPassword)	
...
export async function loginUser(request: Request, response: Response): Promise<void> {
	...
	try {
		...
		if(userID && await bcrypt.compare(password, userPassword)){
			...
			const userArrayWithoutPassword = result.rows.map((obj) => ({user_id: obj.user_id, name: obj.name, email:obj.email, accessToken : accessToken }))
			response.status(200).send(userArrayWithoutPassword)
